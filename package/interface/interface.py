...
for i in range (0, 3):
    for j in range(0, 6):
        map_objets["personnage_" + str(i+1) + "_" + str(j+1) + "liste"] = (ObjetPersonnageListe(longueur*(70 + i * 10)/100 + 25, 65 + j * 50, 50, "images/" + str(j+1) + "_" + str(i) + ".png", [j+1, "Joueuse n°" + str(i+1)]))
        map_objets["statut_" + str(i+1) + "_" + str(j+1) + "liste"] = (ObjetStatut(longueur*(70 + i * 10)/100 + 85, 75 + j * 50, 100, "images/" + "En vie" + "_" + str(i) + ".png", [j+1, "Joueuse n°" + str(i+1), "En vie"]))
...
class Bouton:
    ...
    def __init__(self, position, dimensions, texte, couleur, couleur_texte, alignement="centre", radius=1):
...
map_boutons["boutons_choix_rien"] = BoutonChoix((longueur*90/100 + 30, largeur*83/100 + 40), (longueur*8/100 - 10, largeur*10/100 - 10), "Ne rien faire", (227, 140, 140), (0, 0, 0))
...
for i in range(0, 3):
    for j in range(0, 3):
        map_boutons["zone_" + str(i+3*j+1)] = BoutonZone((40 + i * 420, 40 +j *200), (400, 180), "Zone n°" + str(i+3*j+1), couleurs_zones[i+3*j], (0, 0, 0), "haut_centre", 0)
for i in range(0, 2):
    map_boutons["zone_" + str(i+10)] = BoutonZone((250 + i * 420, 640), (400, 180), "Zone n°" + str(i+10), couleurs_zones[i+9], (0, 0, 0), "haut_centre", 0)


def initialisation_placement(position_zones: list):
    if len(position_zones) != 11:
        raise ValueError("La liste des positions des zones doit contenir 11 éléments")
    for i in range(0, 11):
        map_objets["personnage_" + str(i//5) + "_" + str(i%5 + 1)] = (ObjetFixe)
